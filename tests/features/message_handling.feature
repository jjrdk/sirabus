Feature: Event Handling
  ...
  Scenario Template: Message publishing and subscription
    ...
    When I send a <serializer> <event_type> message to the <broker_type> service bus with TLS disabled
    ...
    Examples:
      ...
      | redis       | pydantic   | test                 |
      | pubsub      | cloudevent | test.test_sub.nested |
      | pubsub      | cloudevent | test                 |
      | pubsub      | pydantic   | test.test_sub.nested |
      | pubsub      | pydantic   | test                 |
  ...
  Scenario Template: Multiple event handling segregation
    ...
    And a <serializer> <broker_type> service bus with TLS disabled is configured with the hierarchical topic map
    When I send a <serializer> <event_type> message to the <broker_type> service bus with TLS disabled
    When I send a <serializer> other message to the <broker_type> service bus with TLS disabled
